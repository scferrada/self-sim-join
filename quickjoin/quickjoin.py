...
def quickjoin_iter(data, r, c, k, join_func, results=None, eps=0, distances=0):
    ...
    while not qw.empty():
        ...
        if (Lw.shape[0] + Gw.shape[0]) <= c:
            results, p_dists, maxdist1 = join_func(Lw, Gw, results, r, k, eps)
            if maxdist1 > maxdist:
                maxdist = maxdist1
            distances += p_dists
            qw.task_done()
        ...
        qw.put((L1, L2))
        qw.put((G1, G2))
        if Lw1.shape[0] != 0 or Gw1.shape[0] != 0:
            qw.put((Lw1, Gw1))
        if Lw2.shape[0] != 0 or Gw2.shape[0] != 0:
            qw.put((Lw2, Gw2))
    q.join()
    qw.join()
    return results, distances, maxdist
...
def quickjoin(data, k, c):
    ...
    results2, distances2, _ = quickjoin_iter(data, maxdist, c, k, knn_join, results)
