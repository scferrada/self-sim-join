...
def knn_join(data1, data2, results, r, k, eps):
	...
	for elem_i in data1:
		...
		for i, dist in enumerate(distances):
			if i == elem_i[0]: continue
			if dist > maxdist: maxdist = dist
			if len(results[elem_i[0]]) < k:
				heappush(results[elem_i[0]], Res(i, dist))
			elif dist < results[elem_i[0]][0].dist:
				heappushpop(results[elem_i[0]], Res(i,dist))
	return d, maxdist
...
def quickjoin_iter(data, r, c, k, join_func, results = None, eps = 0, distances=0):
	...
	while not q.empty():
		slice = q.get()
		if slice.shape[0] <= c:
			p_dists, maxdist1 = join_func(slice, slice, results, r, k, eps)
			distances += p_dists
			q.task_done()
			continue
	...
	while not qw.empty():
		...
		if (Lw.shape[0] + Gw.shape[0]) <= c:
			p_dists, maxdist2 = join_func(Lw, Gw, results, r, k, eps)
			distances += p_dists
			qw.task_done()
			print("cont")
			continue
