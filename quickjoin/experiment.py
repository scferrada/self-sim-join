...
def run_t(id):	
	for j in range(args.iter/MAX_THREADS):
		for i in range(100/args.batch_size):
			...
			res, distances = qj.quickjoin(data, 35, 10, 10, 0.6)
			outfile = open(os.path.join(args.outfile, "%d_%d_%d" % (id,i+1,j+1)),'w')
			for (source, target) in res:
				txt = "%s; %s\n"
				source_id = hashing[source.tostring()]
				target_id = hashing[target.tostring()]
